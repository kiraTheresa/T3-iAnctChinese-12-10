...
export const aiService = {
  ...
  async autoAnnotateRelations(text, entityAnnotations, model = 'xunzi-qwen2') {
    try {
      ...
      // 设计提示词，让AI生成实体间的关系
const prompt = `
你是一项用于古汉语文本分析的“关系抽取模块”。你的任务是根据文本内容和给定实体列表，抽取文本中**明确**出现的关系，并严格按照指定 JSON Schema 输出。
...
说明：
- 实体列表中每一项对应一个实体，其索引为列表顺序（从0开始）。
- 你只能从文本中寻找“明确表达的关系”，不能推测或脑补。

-------------------------------------------------------
【任务要求 — 必须严格遵守】
1. 关系必须直接来自文本中可见、明确的语义，不得依赖常识、不得推测、不填补缺失内容。
2. 你只能根据文本的实际字句判断关系。
3. 关系是“有方向性”的，例如：
   - “A 为 B 之父” → A 指向 B，relationName: "父子关系"
   - “张三谓李四曰” → 张三 指向 李四，relationName: "言语关系"
4. 如果文本中没有明确关系，则输出 `"relations": []`。
5. 禁止输出自反关系（entity1Index == entity2Index）。
6. 禁止输出重复关系（同一对实体、同一关系名只能出现一次）。
7. 禁止输出文本中不存在的称呼、身份或推断出的关系。

-------------------------------------------------------
【古汉语中常见“可能表示关系”的词汇（仅用于识别，不代表必定有关系）】
- 亲属：父、子、兄、弟、妻、臣、君、师、生、徒
- 上下级：命、使、遣、奉、属、从、役、拜
- 交互：与、告、谓、语、问、答、使、令
- 指向：之、其、乃、为、以、若、于
注意：必须文本中“明确”指出实体之间关系才可输出，不能根据这些词进行推断。

-------------------------------------------------------
【输出 JSON 格式（唯一允许输出的内容）】
{
  "relations": [
    {
      "entity1Index": 实体索引（数字）,
      "entity2Index": 实体索引（数字）,
      "relationName": "关系名称（中文短语）"
    }
  ]
}

规范要求：
- 最终输出必须是完整 JSON，不得包含任何注释、解释、分析、额外内容。
- relationName 必须简洁，如：
  - "父子关系"
  - "臣属关系"
  - "言语关系"
...
{
  "relations": [
    { "entity1Index": 0, "entity2Index": 1, "relationName": "言语关系" }
  ]
}
...
- 输出不存在或越界的实体索引
- 输出重复关系记录
- 输出 entity1Index == entity2Index 的记录

-------------------------------------------------------
现在开始执行任务。请严格输出 JSON，不得包含任何非 JSON 内容。
`;
